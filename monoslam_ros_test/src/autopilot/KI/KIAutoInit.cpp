...
bool KIAutoInit::update(const monoslam_ros_test::filter_stateConstPtr statePtr)
{
	...
	if(!resetMap)
	{
		...
		case WAIT_FOR_SECOND:
			...
			controller->setTarget(DronePosition(
					statePtr->x,statePtr->y,statePtr->z,statePtr->yaw));
	...
	else
	{
		...
		case STARTED:	// wait 6s to reach hight.
			if(node->getRelativeTime() - stageStarted > reachHeightMS)
			{
				stageStarted = node->getRelativeTime();
				stage = WAIT_FOR_FIRST;
			}
		...
		case WAIT_FOR_FIRST:	// wait 1s and press space
			if(node->getRelativeTime() - stageStarted > 1000)
			{
		...
		case TOOK_FIRST:	// go [up/down] 1s and press space if was initializing.
			if(node->getRelativeTime() - stageStarted < moveTimeMS)
			{
				if(nextUp)
			...
			else if(node->getRelativeTime() - stageStarted < moveTimeMS+waitTimeMS)
			{
				node->sendControlToDrone(node->hoverCommand);
			}
		...
		case WAIT_FOR_SECOND:
			...
			if(node->getRelativeTime() - stageStarted < 2500)	// wait 2s
			{
				node->sendControlToDrone(node->hoverCommand);
				return false;
			}

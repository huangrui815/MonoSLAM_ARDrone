   ...
   struct KLT_TrackerBase
   {
         KLT_TrackerBase(int nIterations, int nLevels, int levelSkip, int windowWidth)
            : _nIterations(nIterations), _nLevels(nLevels),
              ...
              _margin(5), _convergenceThreshold(0.01f), _SSD_Threshold(1000.0f)
         { }
   ...
   struct KLT_SequenceTrackerConfig
   {
         KLT_SequenceTrackerConfig()
            : nIterations(12), //!< #Iterations in the tracker
              ...
              SSD_Threshold(5000.0f),
              trackWithGain(false),
              minDistance(8), //!< Radius of non-max feature suppression
              minCornerness(1000.0f),
              detectBorderMargin(4.0f) //!< In pixels
         { }

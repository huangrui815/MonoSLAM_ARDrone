      ...
      else
      {
         ...
         //   baseDir = "."; // If environment var is not set, use "."
		  ...
		  const char* baseDir =
			  "c:/SDKs/dlls/shaders";
#else
		  const char* baseDir = "/home/rui/workspace/monoslam-for-ros/src/tracking/CGKLT/Shaders";
#endif
		 #ifdef WIN32
...
namespace V3D_GPU
{
   ...
   Cg_ProgramBase::initializeCg()
   {
      if (_context == 0)
      {
         ...
         cout << "framgementProfile:" << fragmentProfile << endl;
         cout << "vetexProfile:" << vertexProfile << endl;
      }
   }

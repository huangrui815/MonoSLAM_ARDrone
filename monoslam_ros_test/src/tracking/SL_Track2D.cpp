...
int trackGetCorrespondence(const Track2DPtrList& tks , int f1 , int f2 , double* pts1 , double* pts2) {
	...
	for (iter = tks.begin(); iter != tks.end(); ++iter) {
		Track2D* tk = *iter;
		if (tk->f1 <= f1 && tk->f2 >= f2) {
			TrackedFeaturePoint* p = tk->head.next;
			while (p) {
				if (p->pt.f == f1) {
					pts1[cn * 2] = p->pt.x;
					pts1[cn * 2 + 1] = p->pt.y;
...
int trackGetCorrespondence(const Track2DPtrList& tks , int f1 , int f2 , std::vector<TrackedFeaturePoint*>& nodes1 , std::vector<
		TrackedFeaturePoint*>& nodes2) {
	if (f1 >= f2)
		repErr("tracks_get_corresponds() : f1 < f2 is required.");
	Track2DPtrList::const_iterator iter;
...
void trackNodes2Arr(std::vector<TrackedFeaturePoint*>& nodes , int i1 , int i2 , double* pts) {
	size_t i, n = i2 - i1;
	for (i = 0; i < n; ++i) {
		pts[2 * i] = nodes[i1 + i]->pt.x;
		pts[2 * i + 1] = nodes[i1 + i]->pt.y;
	}
}

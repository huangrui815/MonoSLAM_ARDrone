...
protected:
	...
	virtual void OnStart(wxCommandEvent& event) {
		...
		if (SLAMParam::calFilePath.empty()) {
			wxString wxstr = m_filePickerCal->GetPath();
			SLAMParam::calFilePath.resize(1);
			SLAMParam::calFilePath = wxstr.ToStdString();
			if (SLAMParam::calFilePath == "") {
